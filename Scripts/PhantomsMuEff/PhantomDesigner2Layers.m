...
%% Phantoms
muaLayer2   = muaMin* 2.^[1:(numOfPhantoms)];
muEffLayer2 = sqrt(3*muaLayer2.*(muaLayer2 + musPPh));
muaLayer1   = muaLayer2(pivotIdx);
muEffLayer1 = muEffLayer2(pivotIdx);
...
Cinkbase =  (muaLayer2 - muaWater) ./(muaInkBase100 - muaWater);
...
VinkLayer2   = (Cinkbase ./ (1-Cinkbase)) * Vlayer2 ;
VtotalLayer2 = VinkLayer2 + Vlayer2;

VinkLayer1   = Cinkbase(pivotIdx) * Vlayer1 ./ (1-Cinkbase(pivotIdx));
VtotalLayer1 = VinkLayer1 + Vlayer1;

VtotalPivotBase = Vlayer1    * (numOfPhantoms + 1) + Vlayer2;
VinkPivot       = VinkLayer1 * (numOfPhantoms + 1) + VinkLayer2 (pivotIdx);
VtotalPivot     = VtotalPivotBase + VinkPivot; 

fprintf("To create layer 1 (and pivot layer 2) absorptive solution:\n")
fprintf("Mix %.2f ml from the total base \nwith %.4f [ul] of ink base.\n\n", VtotalPivotBase, VinkPivot*1e3);

fprintf("To create Layer 1: \n");
fprintf("Pour %.2f [ml] of pivot absorptive solution into %d beakers\n\n" , Vlayer1, numOfPhantoms);
