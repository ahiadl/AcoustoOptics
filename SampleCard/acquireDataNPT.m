function [bufferDataOut, status] = acquireData(boardHandle, SamplingCard)
AlazarDefs
...
retCode = AlazarBeforeAsyncRead(boardHandle, SamplingCard.channelMask, -int32(SamplingCard.preTriggerSamples), samplesPerBufferPerChannel, 1, hex2dec('7FFFFFFF'), admaFlagd);
admaFlags = ADMA_EXTERNAL_STARTCAPTURE + ADMA_NPT; % Select AutoDMA flags as required
retCode = AlazarBeforeAsyncRead(boardHandle, SamplingCard.channelMask, -int32(SamplingCard.preTriggerSamples), SamplingCard.samplesPerRecord, SamplingCard.trainPerBuffer, hex2dec('7FFFFFFF'), admaFlags);
