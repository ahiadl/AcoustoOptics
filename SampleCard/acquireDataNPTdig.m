function [bufferDataOut, status] = acquireDataNPTdig(digitizer)
AlazarDefs

boardHandle = digitizer.boardHandle;

bufferCount = digitizer.bufferCount;

status = true;
channels = digitizer.channels; %parfor issues;
bufferDataOut = uint16(zeros(digitizer.buffersPerAcquisition, digitizer.samplesPerBuffer));

preparationsTime = tic;
%-----------------------------
%----- Configure AutoDMA -----
%-----------------------------
admaFlagd = ADMA_EXTERNAL_STARTCAPTURE + ADMA_TRIGGERED_STREAMING;
retCode = AlazarBeforeAsyncRead(boardHandle, digitizer.channelMask, -int32(digitizer.preTriggerSamples), samplesPerBufferPerChannel, 1, hex2dec('7FFFFFFF'), admaFlagd);
admaFlags = ADMA_EXTERNAL_STARTCAPTURE + ADMA_NPT; % Select AutoDMA flags as required
retCode = AlazarBeforeAsyncRead(boardHandle, digitizer.channelMask, -int32(digitizer.preTriggerSamples), digitizer.samplesPerRecord, digitizer.trainPerBuffer, hex2dec('7FFFFFFF'), admaFlags);
...
for bufferIndex = 1 : bufferCount
    ...
    retCode = AlazarPostAsyncBuffer(boardHandle, pbuffer, digitizer.bytesPerBuffer);
...
%     fprintf("Buffer Status: %d", bufferFull);
    if bufferFull
%         dataTypeTime = tic;
        setdatatype(bufferOut, 'uint16Ptr', 1, digitizer.samplesPerBuffer); 
...
%         resetBufferTime = tic;
%         setdatatype(bufferOut, 'voidPtr', 1, SamplingCard.samplesPerBuffer); 
        retCode = AlazarPostAsyncBuffer(boardHandle, pbuffer, digitizer.bytesPerBuffer);
...
%         i=i+1;
        ...
        if buffersCompleted >= digitizer.buffersPerAcquisition
            captureDone = true;
        end
    end
